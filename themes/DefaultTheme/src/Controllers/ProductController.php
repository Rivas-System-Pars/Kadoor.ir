...
class ProductController extends Controller
{
    ...
    public function categoryProducts(Category $category)
    {
        ...
        $products  = Product::select('*')->orderByStock()->activeShow()->frontFilter($category)->latest()->whereIn('products.id', $ids)->paginate(20);
    ...
    {
        $products = Product::select('*')->published()
			->activeShow()
    ...
    {
        if (!$product->isShowable() || !$product->is_show) {
            abort(404);
        }
    ...
    {
        $product1 = Product::whereHas('specType')->activeShow()->findOrFail($product1);
        $products[] = $product1;
        ...
        if ($product2) {
            $product2 = Product::whereNotIn('id', $products_id)->activeShow()->where('spec_type_id', $product1->spec_type_id)->findOrFail($product2);
            $products[] = $product2;
        ...
        if ($product3) {
            $product3 = Product::whereNotIn('id', $products_id)->activeShow()->where('spec_type_id', $product1->spec_type_id)->findOrFail($product3);
            $products[] = $product3;
    ...
    public function similarCompare(Request $request)
    {
        ...
        $products = Product::whereIn('id', $products_id)->activeShow()->get();
