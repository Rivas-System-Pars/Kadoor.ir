...
trait ProductScopes
{
    ...
    {
        $query->selectRaw(DB::raw('if ((select max(stock) from prices where prices.product_id = products.id and prices.deleted_at is null) > 0, 1, 0) as in_stock'))->orderBy('in_stock',$type);
    ...
    {
        return $query
            ->selectRaw(DB::raw('(select min(discount_price) from prices where prices.product_id = products.id and prices.deleted_at is null and prices.stock > 0) as min_price'))
